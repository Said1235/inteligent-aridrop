...
`genlayer-js@2.0.0-rc.1` only ships two Studio chains by name — `studionet`
(61999, `studio.genlayer.com`) and `studioDevnet` (61997, but pointed by
default at `studio-dev.genlayer.com`, not `studio-next.genlayer.com`). This
hackathon's own material calls for **Studio Next**, chain **61997**, at
`studio-next.genlayer.com` — a hostname the SDK doesn't know about by name.

Multiple other live GenLayer hackathon projects confirm `studio-next.genlayer.com`
and `studio-dev.genlayer.com` both answer `eth_chainId = 0xf22d` (61997) — the
same chain under two hostnames, not two different networks. This app treats
it that way: **`studioDevnet` is the default here, with its RPC, explorer and
Studio-IDE URLs all pointed at the `studio-next.genlayer.com` hostnames**
instead of the SDK's own `studio-dev.genlayer.com` default. The chain ID,
consensus contract address and everything else about the chain object is
untouched — only the URLs are overridden, in `src/lib/chain.ts`.

To go back to the older `studionet` (61999) chain instead:

```
NEXT_PUBLIC_GENLAYER_NETWORK=studionet
```

To point at neither preset's URLs, override directly:
...
### Confirm this yourself before trusting it

This claim (studio-next and studio-dev being the same chain) was verified by
reading other projects' public READMEs, not by directly querying either RPC —
this sandbox's network egress is restricted to package registries, so neither
`https://studio-next.genlayer.com/api` nor `https://studio.genlayer.com/api`
is reachable from here, and that has been true throughout this project. Run
this yourself, from a machine with normal internet access, before deploying:
...
been deployed to this network at all. If it comes back empty here, try the
same call against `https://studio.genlayer.com/api` before assuming anything
else is wrong; a contract address only exists on the chain it was actually
deployed to, and no frontend configuration changes that.
...
| ------------------------------- | -------------------------------------------- | ------------------------------------------------ |
| `NEXT_PUBLIC_CONTRACT_ADDRESS`  | `0xf3e9c7A8FA961b44f332509d8d79f868f892723A`  | The deployed contract.                            |
| `NEXT_PUBLIC_GENLAYER_NETWORK`  | `studioDevnet`                                | `studioDevnet` (Studio Next) or `studionet`.       |
