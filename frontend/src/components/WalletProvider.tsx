...
type WalletState = {
  ...
  /**
   * True only once the wallet's chain has been positively confirmed to match
   * this app's configured network. Signing must gate on THIS, not on
   * `!wrongNetwork` — while `chainId` is still unresolved (null), the wallet
   * is on an unknown network, and genlayer-js's own chain-mismatch guard
   * (`assertChainMatch`) is a documented no-op for Studio networks
   * (`if (chainConfig.isStudio) return;`), so the SDK provides no fallback
   * protection here. Treating "unknown" as "safe to sign" would fail open.
   */
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<WalletState>(
    () => ({
      ...
      networkConfirmed: status === "connected" && chainId === CHAIN_ID,
      error,
      switching,
